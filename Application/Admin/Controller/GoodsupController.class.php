...
class GoodsUpController extends Controller{
	...
	*/
	public function index(){
		...
		$datatotal = $goodsup->field('count(*) count')->where('issale=1')->find();
		// 计算满足条件的数据总数
		...
		// 连贯操作查询数据
		$goodupdata = $goodsup->field('gid,gpic,gname,goldprice,goodnums,gsellnums,gissuetime')->where('issale=1')->order('gissuetime desc')->limit($limit)->select();
	...
	*/
	public function modGoods(){
		...
		$mod = M('goods');
		// 查询出要修改数据的字段
		$goodsdata = $mod->field('gid,gclassification,gname,gattribute,goldprice,goodnums,gpic,gintroduce,gspecifications,issale')->find($gid);
		// 拆分图片成数组
