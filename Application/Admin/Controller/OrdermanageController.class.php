...
class OrdermanageController extends Controller{
	...
	*/
	public function index(){
		...
		// 全部订单
		// 查询订单信息
		$orders_data = $list->table('sx_member m,sx_orders o')->field('o.oid,o.onumber,o.otime,o.total,o.paymethod,m.uname,o.deliveryaddress')->where('o.uid=m.uid')->limit($limit)->select();
		// 得打发往前台的中间数据，订单详情表
	...
	*/
	public function nonDelivery(){
		...
		$list = M();
		// 订单详情表
		$deliver_data = $list->table('sx_orders o,sx_ordersdetails d,sx_goods g')->field('g.gpic,o.onumber,g.gname,d.gprice,d.gnums,d.ostate,d.id')->where('d.oid=o.oid and d.gid=g.gid and d.ostate=1')->limit($limit)->select();
	...
	public function deliverGoods(){
		...
		$list = M();
		// 订单详情表
		$deliver_data = $list->table('sx_orders o,sx_ordersdetails d,sx_goods g')->field('g.gpic,o.onumber,g.gname,d.gprice,d.gnums,d.ostate,d.id')->where('d.oid=o.oid and d.gid=g.gid and d.ostate=2')->limit($limit)->select();
		foreach ($deliver_data as $key => $value) {
			$deliver_data[$key]['gpic'] = explode(',', $value['gpic'])[0];
		}
		// 全部订单
	...
	public function returnGoods(){
		...
		$detailslist = M("ordersdetails");
		// 遍历出来的是一维数组。select遍历的二维数组。
		$datatotal = $detailslist->field('count(*) count')->where('ostate=3 or ostate=4')->find();
		// 计算满足条件的数据总数
		$totalnums = $datatotal['count'];
		// 实例化分页类
		$pages = new \Think\Page($totalnums,10);
		...
		$list = M();
		// 订单详情表
		$deliver_data = $list->table('sx_orders o,sx_ordersdetails d,sx_goods g')->field('g.gpic,o.onumber,g.gname,d.gprice,d.gnums,d.ostate,d.id')->where('d.oid=o.oid and d.gid=g.gid and (d.ostate=3 or d.ostate=4)')->limit($limit)->select();
		foreach ($deliver_data as $key => $value) {
			$deliver_data[$key]['gpic'] = explode(',', $value['gpic'])[0];
		}
		// 全部订单
	...
	public function completeDeal(){
		...
		$list = M();
		// 订单详情表
		$deliver_data = $list->table('sx_orders o,sx_ordersdetails d,sx_goods g')->field('g.gpic,o.onumber,g.gname,d.gprice,d.gnums,d.ostate,d.id')->where('d.oid=o.oid and d.gid=g.gid and (d.ostate=5 or d.ostate=6)')->limit($limit)->select();
		foreach ($deliver_data as $key => $value) {
			$deliver_data[$key]['gpic'] = explode(',', $value['gpic'])[0];
		}
		// 全部订单
	...
	public function cancleDeal(){
		...
		$list = M();
		// 订单详情表
		$deliver_data = $list->table('sx_orders o,sx_ordersdetails d,sx_goods g')->field('g.gpic,o.onumber,g.gname,d.gprice,d.gnums,d.ostate,d.id')->where('d.oid=o.oid and d.gid=g.gid and d.ostate=0')->limit($limit)->select();
		foreach ($deliver_data as $key => $value) {
			$deliver_data[$key]['gpic'] = explode(',', $value['gpic'])[0];
		}
		// 全部订单
	...
	public function orderState(){
		...
		$this->success('操作成功','/admin.php/admin/Ordermanage/'.$jump.'');
	}
