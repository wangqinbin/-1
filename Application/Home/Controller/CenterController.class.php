...
class CenterController extends Controller{
	public function index(){
		...
		// 近期订单取5条
		$datalist = M();
		$order_data = $datalist->table('sx_orders o,sx_ordersdetails d')->field('d.id,o.onumber,o.otime,o.deliveryaddress,d.gprice,d.gnums,d.ostate')->where("o.oid=d.oid and uid=".$uid)->order('o.otime desc,o.onumber desc')->limit(5)->select();
		foreach ($order_data as $key => $value) {
			$order_data[$key]['deliveryaddress'] = explode('consignee', $value['deliveryaddress'])[1];
		}
		// 近期收藏取5条
		$collection_data = $datalist->table('sx_goods g,sx_goodscollection c')->field('c.id,g.gid,g.gnum,g.gname,g.goldprice')->where('c.gid=g.gid and c.uid='.$uid)->limit(5)->select();
	...
	public function indent(){
		...
		$datalist = M();
		// 计算总条数
		$total =$datalist->table('sx_orders o,sx_ordersdetails d')->field('count(*) count')->where("o.oid=d.oid and o.uid=".$uid)->find();
		$totalnums = $total['count'];
		// 实例化分页类
		$pages = new \Think\Page($totalnums,10);
		// 获取分页的条件
		$limit = $pages->firstRow.','.$pages->listRows;
		$order_data = $datalist->table('sx_orders o,sx_ordersdetails d')->field('d.id,o.onumber,o.otime,o.deliveryaddress,d.gprice,d.gnums,d.ostate')->where("o.oid=d.oid and o.uid=".$uid)->order('o.otime desc,o.onumber desc')->limit($limit)->select();
		foreach ($order_data as $key => $value) {
			$order_data[$key]['deliveryaddress'] = explode('consignee', $value['deliveryaddress'])[1];
		}
	...
	public function collect(){
		...
		$datalist = M();
		// 近期收藏取5条
		$collection_data = $datalist->table('sx_goods g,sx_goodscollection c')->field('c.id,g.gnum,g.gname,g.goldprice')->where('c.gid=g.gid and c.uid='.$uid)->limit(5)->select();
		$this->assign('collectiondata',$collection_data);
