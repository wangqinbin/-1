...
class ShowCartController extends Controller{
	public function index(){
		...
		if ($uid) {
			...
			$list1 = M();
			// 多表查询
			$cartdata = $list1->table('sx_goods g,sx_shoppingcart s')->field('s.gid,g.gpic,g.gname,g.goldprice,s.nums,s.price gprice')->where('s.uid='.$uid.' and s.gid=g.gid')->select();
