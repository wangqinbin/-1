...
class ReviewManageController extends AdminController {
	public function index(){
		...
		// 声明一个空类用来操作多表查询
		$list = M();
		$reviewdata = $list->table('sx_member m,sx_goods g,sx_goodsreview r')->field('r.rid,m.uname,g.gname,r.integral,r.content')->where('r.uid=m.uid and r.gid=g.gid')->limit($limit)->select();
