...
class ArticleManageController extends AdminController{
	public function index(){
		...
		$limit = $page ->firstRow.','.$page->listRows;
                    $list = M();
                    //多表查询语句
                    $data = $list ->table("sx_articles ar,sx_articleclasses ac")->where('ar.pacid = ac.acid')->field('ar.*,ac.acname')->limit($limit)->select();
	...
	public function add(){
		...
		$atitle = $_POST['atitle'];
		$_POST['atime'] = time();
		$res = $articles -> where(array("atitle"=>$atitle)) -> field("atitle") -> select();
		if($res){
			$this -> error("文章标题已存在");
		}else{
			if($articles->create()){
    ...
    public function update(){
    	$articles = M("articles");
           $_POST['atime'] = time();
    	if($articles-> where(array("aid"=>$_POST['aid']))->save($_POST)){
    ...
    public function search(){
            ...
            $list = M();
            //多表查询语句
            $data1 = $list ->table("sx_articles ar,sx_articleclasses ac")->where("ar.atitle $map[0] '$map[1]' and ar.pacid=ac.acid")->field('ar.*,ac.acname')->limit($limit)->select();  
            $this -> assign("page",$page->show());
